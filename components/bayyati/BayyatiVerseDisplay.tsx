...
import BayyatiVerseItem from './BayyatiVerseItem';

interface BayyatiVerseDisplayProps {
  verses: Verse[];
  highlightedVerseId: string | null;
  surahName?: string; // Optional prop for Surah name
}

const BayyatiVerseDisplay: React.FC<BayyatiVerseDisplayProps> = ({ verses, highlightedVerseId, surahName = "As-Saffat" }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-1 md:p-3 border border-gray-200 h-full flex flex-col">
      ...
      <div className="flex-grow p-2 md:p-4 space-y-3 bayyati-container scrollbar-thin scrollbar-thumb-violet-500 scrollbar-track-indigo-950-50"> {/* Added scrollbar classes here, applying to verse list */}
        {verses.map((verse) => (
          <BayyatiVerseItem
            key={verse.id}
            verse={verse}
...
export default BayyatiVerseDisplay;